      ...
      <div className="space-y-5">
        <div className="rounded-2xl border border-secondary-100 bg-secondary-50 p-4">
          <p className="text-sm font-semibold text-secondary-700">
            {reminder.isRecurring ? 'Elige cómo quieres eliminar esta recurrencia.' : 'Vas a eliminar este recordatorio.'}
          </p>
        ...
        <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
          ...
          <div className="mt-2 text-xs text-slate-500">
            {format(parseISO(reminder.occurrenceDate), 'dd MMM yyyy', { locale: es })}
            {reminder.occurrenceDueTime && ` • ${reminder.occurrenceDueTime.slice(0, 5)}`}
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          >
            {isSubmitting === 'single'
              ? 'Eliminando...'
              : reminder.isRecurring ? 'Eliminar solo esta ocurrencia' : 'Eliminar recordatorio'}
          </button>
          ...
          {reminder.isRecurring && (
            ...
            >
              {isSubmitting === 'series' ? 'Cortando serie...' : 'Eliminar esta y las siguientes'}
            </button>
          )}
...
function TaskItem({
  ...
  const isOverdue = !reminder.completed && isAfter(new Date(), parseISO(reminder.occurrenceDate));
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl">
      ...
      <div
        className={cn(
          'bg-white p-4 rounded-2xl border transition-all flex items-center gap-4 shadow-sm group touch-pan-y',
          reminder.completed ? 'opacity-60 border-slate-100' : 'border-slate-200',
          isOverdue && !reminder.completed ? 'border-secondary-200 bg-secondary-50/30' : ''
        )}
      ...
      >
        <button
          ...
          className={cn(
            ...
            reminder.completed ? 'text-primary-600' : 'text-slate-300 hover:text-primary-400'
          )}
        >
        ...
        <div className="flex-1 min-w-0">
          <div className={cn('font-medium text-sm', reminder.completed && 'line-through text-slate-400')}>
            {reminder.title}
          </div>
          <div className={cn(
            'text-xs flex items-center gap-1 mt-1 flex-wrap',
            isOverdue && !reminder.completed && 'text-secondary-600 font-medium',
            !isOverdue || reminder.completed ? 'text-slate-400' : ''
          )}>
...
export function TasksView() {
  ...
  const [archiveConfirmReminderId, setArchiveConfirmReminderId] = useState<string | null>(null);
  ...
  const deleteCandidate = useMemo(
    () => tasks.find(task => task.id === deleteCandidateId) || selectedReminder,
  ...
  const handleTransactionForReminder = async (transaction: Omit<Transaction, 'id' | 'household_id' | 'created_by'>) => {
    ...
    if (reminderCompleted) {
      ...
      if (selectedReminderId === transactionReminder.id) setSelectedReminderId(null);
    }
    ...
    >
      <div className="space-y-4">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex items-center gap-2">
            {(['month', 'week'] as ReminderPeriod[]).map(option => (
              <button
                key={option}
                type="button"
                onClick={() => {
          ...
          <div className="mt-4 flex items-center justify-between gap-3">
            ...
            <div className="min-w-0 text-center">
              <div className="text-xs font-bold uppercase tracking-wide text-slate-400">
                {period === 'month' ? 'Periodo mensual' : 'Periodo semanal'}
              </div>
              <div className="mt-1 text-base font-bold text-slate-800 capitalize">{rangeLabel}</div>
            </div>
      ...
      {tasks.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-slate-200 bg-white p-8 text-center shadow-sm">
          <h3 className="text-lg font-bold text-slate-800">No hay recordatorios en este periodo</h3>
          <p className="mt-2 text-sm text-slate-500">
            Cambia entre semana y mes o navega al siguiente periodo para revisar más ocurrencias.
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          {tasks.map(reminder => (
            <TaskItem
              ...
              onDeleteRequest={() => setDeleteCandidateId(reminder.id)}
      ...
      >
        {selectedReminder && (
          <div className="space-y-5">
            <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
              <h3 className="text-xl font-bold text-slate-800">{selectedReminder.title}</h3>
              <div className="mt-3 space-y-2 text-sm text-slate-600">
                <div className="flex items-center gap-2">
                  <Calendar className="w-4 h-4 text-primary-500" />
                  <span>
                    {format(parseISO(selectedReminder.occurrenceDate), 'dd MMMM yyyy', { locale: es })}
                    {selectedReminder.occurrenceDueTime && ` • ${selectedReminder.occurrenceDueTime.slice(0, 5)}`}
                  </span>
                </div>
                {selectedReminder.isRecurring && selectedReminder.recurrenceLabel && (
                  <div className="flex items-center gap-2">
                    <Repeat className="w-4 h-4 text-primary-500" />
                    <span>{selectedReminder.recurrenceLabel}</span>
                  </div>
                )}
                {selectedReminder.completed && selectedReminder.completedAt && (
                  <div className="flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-primary-500" />
                    <span>Completado el {format(parseISO(selectedReminder.completedAt), 'dd MMM yyyy, HH:mm', { locale: es })}</span>
                  </div>
                )}
            ...
            <div className="grid grid-cols-1 gap-3">
              <button
                ...
                onClick={async () => {
                  ...
                  if (!selectedReminder.completed && selectedReminder.requiresTransaction) {
                    setTransactionReminderId(selectedReminder.id);
                    return;
                  }

                  const wasUpdated = selectedReminder.completed
                    ? await reopenReminder(selectedReminder)
                    : await completeReminder(selectedReminder);
              ...
              >
                {selectedReminder.completed ? 'Reabrir esta ocurrencia' : 'Completar esta ocurrencia'}
              </button>

              <button
                type="button"
                onClick={() => setEditingReminderId(selectedReminder.id)}
              ...
              >
                <span className="inline-flex items-center gap-2">
                  ...
                  {selectedReminder.isRecurring ? 'Editar serie' : 'Editar recordatorio'}
                </span>
              </button>

              <button
                type="button"
                onClick={() => setDeleteCandidateId(selectedReminder.id)}
              ...
              >
                <span className="inline-flex items-center gap-2">
                  <Trash2 className="w-4 h-4" />
                  {selectedReminder.isRecurring ? 'Eliminar ocurrencia o serie' : 'Eliminar recordatorio'}
                </span>
              </button>
              ...
              {selectedReminder.isRecurring && (
                <button
                  type="button"
                  onClick={() => setArchiveConfirmReminderId(selectedReminder.id)}
      ...
      <DeleteReminderModal
        ...
        onDeleteSingle={async () => {
          ...
          if (wasDeleted && selectedReminderId === deleteCandidate.id) setSelectedReminderId(null);
          return wasDeleted;
        }}
        onDeleteSeries={async () => {
          if (!deleteCandidate) return false;
          const wasDeleted = await deleteSeriesFromReminder(deleteCandidate);
          if (wasDeleted && selectedReminderId === deleteCandidate.id) setSelectedReminderId(null);
          return wasDeleted;
        }}
      />
      ...
      <TaskFormModal
        ...
        onSubmit={async (task) => {
          if (!editingReminder) return { success: false, error: 'No se encontró el recordatorio.' };

          const result = await updateTask(editingReminder.taskId, task);
          if (result.success) {
        ...
        initialTask={editingReminder ? buildTaskInputFromTask(editingReminder.task) : undefined}
        title={editingReminder?.isRecurring ? 'Editar serie' : 'Editar recordatorio'}
          ...
          <div className="space-y-5">
            <div className="rounded-2xl border border-amber-100 bg-amber-50 p-4">
              ...
              <p className="mt-2 text-sm text-amber-600">
                No se generarán nuevas ocurrencias después de hoy, pero el historial visible se conservará.
              </p>
            </div>

            <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
              <div className="text-sm font-bold text-slate-800">{archiveConfirmReminder.title}</div>
              {archiveConfirmReminder.recurrenceLabel && (
                <div className="mt-2 text-xs text-slate-500">{archiveConfirmReminder.recurrenceLabel}</div>
              )}
            </div>
            ...
            <div className="flex gap-3">
              <button
                type="button"
                onClick={() => setArchiveConfirmReminderId(null)}
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  const wasArchived = await archiveTaskSeries(archiveConfirmReminder.taskId);
                  if (wasArchived) {
                    setArchiveConfirmReminderId(null);
                    if (selectedReminderId === archiveConfirmReminder.id) setSelectedReminderId(null);
                  }
                }}
      ...
      <TransactionModal
        ...
        accounts={accounts}
        initialDescription={transactionReminder ? transactionReminder.title : ''}
