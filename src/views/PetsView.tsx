...
function PendingPetTaskItem({
    ...
    return (
        <div className="relative overflow-hidden rounded-xl">
            ...
            <div
                className={cn(
                    "bg-slate-50 p-3 rounded-xl flex justify-between items-center transition-transform group",
            ...
            >
                <button
                    type="button"
                    onClick={onComplete}
                    className="p-2 bg-emerald-500 text-white rounded-lg shadow-sm mr-2 shrink-0"
                ...
                >
                    ...
                    <div className="text-[10px] text-slate-500 flex items-center gap-1">
                        ...
                        {format(parseISO(task.scheduled_date), "dd MMM yyyy", {
                            locale: es,
                        })}
            ...
            <div className="space-y-8">
                {pets.map((pet) => {
                    const tasksForPet = petTasks.filter(
                        (t) => t.pet_id === pet.id && !t.completed,
                    );
                    const historyForPet = petTasks.filter(
                        (t) => t.pet_id === pet.id && t.completed,
                    );
                    const displayHistory = [...historyForPet]
                        .sort((a, b) =>
                            (b.completed_date || "").localeCompare(
                                a.completed_date || "",
                            ),
                        )
                        .slice(0, 3);
                            ...
                            <div className="space-y-4 mb-6">
                                <div className="flex items-center gap-3 text-sm text-slate-600">
                                    ...
                                    <span>
                                        ...
                                        {format(
                                            parseISO(
                                                activeSelectedTask.scheduled_date,
                                            ),
                                            "dd MMMM yyyy",
                                            { locale: es },
                                        )}{" "}
                                        {activeSelectedTask.scheduled_time}
                                    </span>
                                </div>

                                {activeSelectedTask.completed &&
                                    activeSelectedTask.completed_date && (
                                        <>
